...
INSTALLER_PATH = ROOT / "extras" / "claude-code" / "tools" / "install.py"
...
def test_cmd_install_defers_datacloud_runtime_until_updated_installer_restarts(
    ...
    def fake_download_repo_zip(target: Path) -> bool:
        ...
        (source_dir / "extras" / "claude-code" / "shared" / "hooks").mkdir(parents=True)
        (source_dir / "extras" / "claude-code" / "shared" / "lsp-engine").mkdir(parents=True)
        (source_dir / "extras" / "claude-code" / "shared" / "code_analyzer").mkdir(parents=True)
        (source_dir / "skills").mkdir(parents=True)
        (source_dir / "extras" / "claude-code" / "tools").mkdir(parents=True)
        (source_dir / "extras" / "claude-code" / "shared" / "hooks" / "skills-registry.json").write_text('{"version": "9.9.9"}')
        (source_dir / "extras" / "claude-code" / "tools" / "install.py").write_text("# updated installer bytes\n")
        return True
    ...
    monkeypatch.setattr(
        _install,
        "get_datacloud_runtime_status",
        lambda: {"installed": False, "available": False, "details": []},
    )
