...
**Start here:** [Available Skills](#available-skills) · [Installation](#installation) · [Repository Layout](#repository-layout) · [Claude Code Features](#claude-code-features) · [Skill Architecture](#skill-architecture)

## Repository Layout

- `skills/` is the primary source of truth for all reusable Salesforce skills.
- `.codex-plugin/plugin.json` makes the repo root installable as a Codex plugin.
- `.agents/plugins/marketplace.json` provides a repo-local marketplace entry that points back to this plugin root.
...
This repository is now a plugin root. The Codex-facing plugin files live at:

- [`.codex-plugin/plugin.json`](.codex-plugin/plugin.json)
- [`.agents/plugins/marketplace.json`](.agents/plugins/marketplace.json)

That keeps `skills/` portable while still making the repo installable as a plugin in Codex-oriented environments.
...
> After sf-skills is installed, use the installed copy at `~/.claude/sf-skills-install.py` for normal updates. Use `extras/claude-code/tools/install.py` only when developing or testing from a cloned repo checkout.
>
...
For deeper install and hook internals, see [extras/claude-code/tools/README.md](extras/claude-code/tools/README.md) and [extras/claude-code/shared/hooks/README.md](extras/claude-code/shared/hooks/README.md).
...
Skills are available as slash commands (for example `/sf-apex`, `/sf-flow`, `/sf-ai-agentscript`). Claude can also select the appropriate skill dynamically from your request context — keywords, intent, and file patterns in `extras/claude-code/shared/hooks/skills-registry.json` document what each skill is best at.
...
All PostToolUse validations are routed through a central dispatcher (`extras/claude-code/shared/hooks/scripts/validator-dispatcher.py`) that receives file paths from Write/Edit hook context, matches file patterns to determine which validators to run, and returns combined validation output.
...
Seven specialized Claude Code agents for Salesforce implementations, sourced from `extras/claude-code/agents/` and installed to `~/.claude/agents/`.
...
| 🧱 | `sf-industry-cme-epc-model` | CME EPC Product2 modeling for products, attributes, and offer bundles | ✅ Live |
...
3. Make your changes
4. Test locally: `python3 extras/claude-code/tools/install.py --dry-run`
...
| [Shreyas Dhond, CTA (ShreyasD)](https://github.com/ShreyasD) | Primary contributor — Industries CME EPC modeling | sf-industry-cme-epc-model |
