...
```
extras/claude-code/shared/hooks/
...
1. Verify the hook scripts exist and are executable:
   ```bash
   /bin/ls -la extras/claude-code/shared/hooks/scripts/validator-dispatcher.py
   ```
...
1. Edit the prompt hook text in `extras/claude-code/tools/install.py` `get_hooks_config()` or directly in `~/.claude/settings.json`
